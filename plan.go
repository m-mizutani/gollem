...
type Plan struct {
	...
	todos                  []planToDo
	state                  PlanState
	directResponse         string // Response for direct_response approach
...
// ClarificationResponse represents the response from goal clarifier
...
type planConfig struct {
	...
	oldPlan                   *Plan                         // Reference to existing plan for updates
...
func (g *Agent) Plan(ctx context.Context, prompt string, options ...PlanOption) (*Plan, error) {
	...
	if clarification.Approach == "direct_response" {
		return g.handleDirectResponse(ctx, planID, prompt, clarification, toolMap, cfg, logger)
	}

	// Handle update_plan approach
	if clarification.Approach == "update_plan" && cfg.oldPlan != nil {
		return g.handleUpdatePlan(ctx, planID, prompt, clarification, toolMap, toolList, cfg, logger)
	}

	// For new_plan approach (or fallback), proceed with traditional planning
	return g.handleNewPlan(ctx, planID, prompt, clarification.ClarifiedGoal, toolMap, toolList, cfg, logger)
}
...
func (g *Agent) handleDirectResponse(ctx context.Context, planID, prompt string, clarification *ClarificationResponse, toolMap map[string]Tool, cfg *planConfig, logger *slog.Logger) (*Plan, error) {
	...
	plan := &Plan{
		...
		directResponse: clarification.Response,
		state:          PlanStateCompleted,
		todos:          []planToDo{}, // No todos for direct response
		agent:          g,
		toolMap:        toolMap,
		config:         cfg,
		logger:         logger,
...
// handleUpdatePlan handles the update_plan approach where an existing plan is modified
func (g *Agent) handleUpdatePlan(ctx context.Context, planID, prompt string, clarification *ClarificationResponse, toolMap map[string]Tool, toolList []Tool, cfg *planConfig, logger *slog.Logger) (*Plan, error) {
	logger.Debug("executing update_plan approach",
		"old_goal", cfg.oldPlan.clarifiedGoal,
		"new_goal", clarification.ClarifiedGoal)

	// Create updated plan structure
	updatedPlan := &Plan{
		id:                     cfg.oldPlan.id,                     // Keep same ID for continuity
	...
	// Copy completed todos from old plan
	for _, todo := range cfg.oldPlan.todos {
		if todo.Status == ToDoStatusCompleted {
			updatedPlan.todos = append(updatedPlan.todos, todo)
		}
	}

	// Use reflector to update the plan
	if err := g.updatePlanWithReflector(ctx, updatedPlan, clarification, cfg); err != nil {
		return nil, err
	}

	// Create main session for the updated plan
	mainSession, err := g.createMainSessionForPlan(ctx, updatedPlan, toolList, cfg)
	if err != nil {
		return nil, goerr.Wrap(err, "failed to create main session for updated plan")
	}
	updatedPlan.mainSession = mainSession

	// Call PlanCreatedHook
	if err := cfg.planCreatedHook(ctx, updatedPlan); err != nil {
		return nil, goerr.Wrap(err, "failed to call PlanCreatedHook for updated plan")
	}

	return updatedPlan, nil
}

// handleNewPlan handles the new_plan approach where a fresh plan is created
func (g *Agent) handleNewPlan(ctx context.Context, planID, prompt, clarifiedGoal string, toolMap map[string]Tool, toolList []Tool, cfg *planConfig, logger *slog.Logger) (*Plan, error) {
...
// updatePlanWithReflector uses the reflector to update an existing plan
func (g *Agent) updatePlanWithReflector(ctx context.Context, plan *Plan, clarification *ClarificationResponse, cfg *planConfig) error {
	// Create reflector session
	reflectorSession, err := g.createReflectorSession(ctx, cfg)
	if err != nil {
		return goerr.Wrap(err, "failed to create reflector session for plan update")
	}

	// Build reflector template data with goal update context
	reflectorData := reflectorTemplateData{
		Goal:                   clarification.ClarifiedGoal,
		ClarifiedGoal:          clarification.ClarifiedGoal,
		SimplifiedSystemPrompt: cfg.oldPlan.simplifiedSystemPrompt,
		CurrentPlanStatus: fmt.Sprintf(`IMPORTANT: The user has updated the goal.
Original goal: %s
New goal: %s

Please update the plan to achieve the NEW GOAL while leveraging completed work where applicable.`,
			cfg.oldPlan.clarifiedGoal, clarification.ClarifiedGoal),
		OriginalPlan:   formatOriginalPlan(cfg.oldPlan.todos),
		PendingTodos:   formatPendingTodos(cfg.oldPlan.todos),
		CompletedSteps: formatCompletedSteps(cfg.oldPlan.todos),
		LastStepResult: fmt.Sprintf(`User requested goal update:
- Original goal: %s
- New goal: %s
- Reason: %s`, cfg.oldPlan.clarifiedGoal, clarification.ClarifiedGoal, clarification.Reasoning),
		Language: cfg.language,
	}

	// Execute reflector to get updated plan
	var promptBuffer bytes.Buffer
	if err := reflectorTmpl.Execute(&promptBuffer, reflectorData); err != nil {
		return goerr.Wrap(err, "failed to execute reflector template for plan update")
	}

	response, err := reflectorSession.GenerateContent(ctx, Text(promptBuffer.String()))
	if err != nil {
		return goerr.Wrap(err, "failed to get reflector response for plan update")
	}

	if len(response.Texts) == 0 {
		return goerr.New("no response from reflector for plan update")
	}

	// Parse reflector response
	var reflection planReflection
	if err := json.Unmarshal([]byte(response.Texts[0]), &reflection); err != nil {
		return goerr.Wrap(err, "failed to parse reflector response for plan update")
	}

	// Apply reflector updates to the plan
	return plan.applyReflectorUpdates(ctx, &reflection)
}

// createMainSessionForPlan creates the main session for plan execution
func (g *Agent) createMainSessionForPlan(ctx context.Context, plan *Plan, toolList []Tool, cfg *planConfig) (Session, error) {
	...
	if plan.simplifiedSystemPrompt != "" {
		sessionOptions = append(sessionOptions, WithSessionSystemPrompt(plan.simplifiedSystemPrompt))
	}
...
func (p *Plan) Execute(ctx context.Context) (string, error) {
	...
	// Handle direct response plans
	if p.directResponse != "" && p.state == PlanStateCompleted {
		logger.Debug("executing direct response plan")
		// Call PlanCompletedHook
		if err := p.config.planCompletedHook(ctx, p, p.directResponse); err != nil {
			return "", goerr.Wrap(err, "failed to call PlanCompletedHook for direct response")
		}
		return p.directResponse, nil
	}
...
func (g *Agent) createPlanWithRuntime(ctx context.Context, id, input, clarifiedGoal, simplifiedSystemPrompt, directResponse string, todos []planToDo, state PlanState, toolMap map[string]Tool, toolList []Tool, cfg *planConfig) (*Plan, error) {
	logger := ctxlog.From(ctx)
...
// clarifyGoalWithRetry clarifies the user's goal and determines the best approach with retry logic
func (g *Agent) clarifyGoalWithRetry(ctx context.Context, userInput string, cfg *planConfig) (*ClarificationResponse, error) {
	logger := ctxlog.From(ctx)
	maxRetries := 3

	for attempt := 0; attempt < maxRetries; attempt++ {
		// Run goal clarifier
		result, err := g.runGoalClarifier(ctx, userInput, cfg)
		if err != nil {
			return nil, err
		}

		// Parse JSON response
		var clarification ClarificationResponse
		if err := json.Unmarshal([]byte(result), &clarification); err != nil {
			if attempt < maxRetries-1 {
				logger.Debug("Failed to parse clarification response, retrying",
					"result", result,
					"attempt", attempt+1,
					"error", err,
				)
			...
			return nil, fmt.Errorf("failed to parse clarification after %d attempts: %w", maxRetries, err)
		}

		// Validate response
		if err := clarification.Validate(cfg.oldPlan != nil); err != nil {
			if attempt < maxRetries-1 {
				logger.Debug("Invalid clarification response, retrying",
					"result", result,
					"attempt", attempt+1, "error", err)
				continue
			}
			return nil, fmt.Errorf("invalid clarification after %d attempts: %w", maxRetries, err)
		}

		return &clarification, nil
	}

	return nil, fmt.Errorf("failed to get valid clarification after %d attempts", maxRetries)
}
...
func (g *Agent) runGoalClarifier(ctx context.Context, userInput string, cfg *planConfig) (string, error) {
	...
	// Generate tool info
	toolInfo := g.generateToolInfo()

	// Generate old plan info if provided
	var oldPlanInfo *goalClarifierOldPlanInfo
	if cfg.oldPlan != nil {
		completedCount := 0
		for _, todo := range cfg.oldPlan.todos {
			if todo.Status == ToDoStatusCompleted {
		...
		oldPlanInfo = &goalClarifierOldPlanInfo{
			Goal:           cfg.oldPlan.clarifiedGoal,
			Progress:       fmt.Sprintf("%.0f%%", float64(completedCount)/float64(len(cfg.oldPlan.todos))*100),
			CompletedCount: completedCount,
			TotalCount:     len(cfg.oldPlan.todos),
			Status:         string(cfg.oldPlan.state),
		}
	}
...
// formatOriginalPlan formats the original plan todos for reflector
func formatOriginalPlan(todos []planToDo) string {
...
// formatPendingTodos formats pending todos for reflector
func formatPendingTodos(todos []planToDo) string {
	var sb strings.Builder
	for _, todo := range todos {
		if todo.Status != ToDoStatusCompleted {
...
// formatCompletedSteps formats completed todos for reflector
func formatCompletedSteps(todos []planToDo) string {
	var sb strings.Builder
	for _, todo := range todos {
		if todo.Status == ToDoStatusCompleted {
			sb.WriteString(fmt.Sprintf("- %s\n", todo.Description))
...
// applyReflectorUpdates applies updates from reflection to the plan
func (p *Plan) applyReflectorUpdates(ctx context.Context, reflection *planReflection) error {
	// Apply new todos
	for _, newTodo := range reflection.NewToDos {
		// Generate new ID for new todos
		newTodo.ID = uuid.New().String()
		newTodo.CreatedAt = time.Now()
		newTodo.Status = ToDoStatusPending
		p.todos = append(p.todos, newTodo)
	}

	// Apply updated todos (replace existing ones)
	for _, updatedTodo := range reflection.UpdatedToDos {
		for i, todo := range p.todos {
			if todo.ID == updatedTodo.ID {
				// Keep original timestamps but update other fields
				updatedTodo.CreatedAt = todo.CreatedAt
				p.todos[i] = updatedTodo
				break
			}
		}
	}
	...
	for _, skip := range reflection.SkipDecisions {
		for i, todo := range p.todos {
			if todo.ID == skip.TodoID {
	...
	// Apply legacy skipped todos
	for _, skippedID := range reflection.SkippedToDos {
		for i, todo := range p.todos {
			if todo.ID == skippedID {
				p.todos[i].Status = ToDoStatusSkipped
				break
			}
		}
...
// generateToolInfo generates a string representation of available tools for goal clarifier
func (g *Agent) generateToolInfo() string {
	var toolInfo strings.Builder
	toolInfo.WriteString("Available tools:\n")

	// Add tools from g.tools
	for _, tool := range g.tools {
		spec := tool.Spec()
		toolInfo.WriteString(fmt.Sprintf("- %s: %s\n", spec.Name, spec.Description))
	}

	// Add tools from g.toolSets
	if len(g.toolSets) > 0 {
		// Note: We can't call Specs here because it requires context
		// For goal clarifier, we'll just note that tool sets are available
		toolInfo.WriteString("- [Additional tools from tool sets]\n")
	}

	if len(g.tools) == 0 && len(g.toolSets) == 0 {
		toolInfo.WriteString("- No tools available\n")
	}
...
func (p *Plan) MarshalJSON() ([]byte, error) {
	data := planData{
		...
		ToDos:                  p.todos,
		State:                  p.state,
		DirectResponse:         p.directResponse,
	}
...
func (g *Agent) NewPlanFromData(ctx context.Context, data []byte, options ...PlanOption) (*Plan, error) {
	...
	plan, err := g.createPlanWithRuntime(ctx, planData.ID, planData.Input, planData.ClarifiedGoal, planData.SimplifiedSystemPrompt, planData.DirectResponse, planData.ToDos, planData.State, toolMap, toolList, cfg)
	if err != nil {
...
func (p *Plan) UnmarshalJSON(data []byte) error {
	...
	p.directResponse = planData.DirectResponse
...
// WithOldPlan sets an existing plan to be used as reference for updates.
// When provided, the goal clarifier can choose to update the existing plan
// instead of creating a new one.
func WithOldPlan(oldPlan *Plan) PlanOption {
	return func(cfg *planConfig) {
		cfg.oldPlan = oldPlan
	}
}
